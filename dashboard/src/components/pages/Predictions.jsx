...
export default function Predictions() {
  ...
  const fetchTempPreds = async () => {
    try {
      ...
      setTempPreds(prev => ({ ...prev, loading: false, error: err.message || "Failed to load Delhi temperature predictions." }));
    }
  };
  ...
  const fetchHumidityPreds = async () => {
    try {
      ...
      setHumidityPreds(prev => ({ ...prev, loading: false, error: err.message || "Failed to load regional humidity predictions." }));
    }
  };
  ...
  const fetchNetworkPreds = async () => {
    try {
      ...
      setNetworkPreds(prev => ({ ...prev, loading: false, error: err.message || "Failed to load network GB predictions." }));
    }
  };
  ...
  // Determine active state indicators based on current mode
  let activeData = [];
  let isLoading = false;
  let isError = null;
  let retryFn = () => {};
  ...
  if (chartMode === 'temp') {
    activeData = tempPreds.data;
    isLoading = tempPreds.loading;
    isError = tempPreds.error;
    retryFn = fetchTempPreds;
    unit = "°C";
    title = "Temperature Forecasting";
    modelType = "Linear Regression";
    modelColor = "#8b5cf6";
  } else if (chartMode === 'humidity') {
    activeData = humidityPreds.data;
    isLoading = humidityPreds.loading;
    isError = humidityPreds.error;
    retryFn = fetchHumidityPreds;
    unit = "%";
    title = "Humidity Forecasting";
    modelType = "Linear Regression";
    modelColor = "#06b6d4";
  } else if (chartMode === 'battery') {
    activeData = networkPreds.data?.battery ? [...networkPreds.data.battery].sort((a, b) => a.unix_ts - b.unix_ts) : [];
    isLoading = networkPreds.loading;
    isError = networkPreds.error;
    retryFn = fetchNetworkPreds;
    unit = "%";
    title = "Battery Decay Prediction";
    modelType = "Gradient Boosting";
    modelColor = "#10b981";
  } else if (chartMode === 'latency') {
    activeData = networkPreds.data?.latency ? [...networkPreds.data.latency].sort((a, b) => a.unix_ts - b.unix_ts) : [];
    isLoading = networkPreds.loading;
    isError = networkPreds.error;
    retryFn = fetchNetworkPreds;
    unit = "ms";
    ...
    activeData = networkPreds.data?.packet_loss ? [...networkPreds.data.packet_loss].sort((a, b) => a.unix_ts - b.unix_ts) : [];
    isLoading = networkPreds.loading;
    isError = networkPreds.error;
    retryFn = fetchNetworkPreds;
    unit = "%";
  ...
  const activeStats = calculateStats(activeData);
  ...
  return (
    <div className="flex flex-col gap-8 w-full text-slate-200">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Comparisons between actual WSN telemetry metrics and values forecasted by the predictive engines.</p>
      </div>

      {/* Toggles switcher controls */}
        ...
        <div className="glass-card px-6 py-4 flex items-center gap-8 self-stretch">
          ...
          {isLoading && !activeData.length ? (
            <div className="flex gap-8 animate-pulse">
              <div className="h-10 w-24 bg-slate-900 rounded" />
              <div className="h-10 w-24 bg-slate-900 rounded" />
            </div>
          ) : (
            <div className="flex gap-8">
              <div className="flex flex-col">
                ...
                <span className="text-base font-bold font-mono text-white" style={{ color: modelColor }}>{activeStats.mae} {unit}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-500 font-semibold uppercase">RMSE (Std Dev)</span>
                <span className="text-base font-bold font-mono text-slate-300">{activeStats.rmse} {unit}</span>
              </div>
            </div>
          )}
        </div>
      ...
      {isLoading && activeData.length === 0 ? (
        ...
        <ErrorCard 
          title="Predictive Analytics Chart Offline" 
          message={isError} 
          onRetry={retryFn}
        />
      ) : (
      ...
      {isLoading && activeData.length === 0 ? (
        ...
        <ErrorCard 
          title="Predictions Analysis Grid Offline" 
          message={isError} 
          onRetry={retryFn}
        />
      ) : (
        <div className="glass-card p-6 flex flex-col gap-4 font-mono">
          <h3 className="text-lg font-bold text-slate-200 m-0 font-sans">Recent Predictions Analysis Table</h3>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-slate-900 text-slate-400 text-xs uppercase tracking-wider font-semibold sticky top-0 bg-slate-950/90 py-2.5 z-10 font-mono">
                  ...
                  <th className="py-2.5 px-4 font-sans">Fit Accuracy</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-900 text-slate-300">
                {activeData.map((row, idx) => {
                  ...
                  if (absErr > lowThreshold) {
                    ratingText = "Low Fit";
                    ratingClass = "bg-rose-500/10 text-rose-400 border-rose-500/25 border font-sans";
                  } else if (absErr > modThreshold) {
                    ratingText = "Moderate Fit";
                    ratingClass = "bg-amber-500/10 text-amber-400 border-amber-500/25 border font-sans";
                  } else {
                    ratingClass = "bg-emerald-500/10 text-emerald-400 border-emerald-500/25 border font-sans";
                  }
                  ...
                  return (
                    <tr key={idx} className="hover:bg-slate-900/40 transition-colors duration-150">
                      ...
                      <td className="py-3 px-4 font-bold text-white font-mono">{row.actual.toFixed(2)}</td>
                      <td className="py-3 px-4 font-semibold font-mono" style={{ color: modelColor }}>{row.predicted.toFixed(2)}</td>
                      <td className={`py-3 px-4 font-mono font-medium ${errorVal >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                        {errorVal >= 0 ? '+' : ''}{errorVal.toFixed(4)}
                      </td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${ratingClass}`}>
                          {ratingText}
                        </span>
                      </td>
                    </tr>
